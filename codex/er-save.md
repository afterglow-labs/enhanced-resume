...
3. Record it in the index FIRST — this creates (and, if the name was already
   ...
   into. Run (adjust the glob to the installed plugin location):
   `python3 ~/.claude/plugins/cache/*/enhanced-resume/bin/ervault add --name "<name>" --agent codex --id "<uuid>" --cwd "<pwd>" --branch "<branch>" --oneliner "<summary>"`
